...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/06/03 11:39:25 by jadelgad          #+#    #+#             */
/*   Updated: 2026/06/03 11:39:26 by jadelgad         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	convert_list_to_array(t_data *data)
{
	...
	if (data->map_list == NULL || data->map.height == 0)
		return (print_error("File without map"));
	data->map.grid = malloc((data->map.height + 1) * sizeof(char *));
	if (!data->map.grid)
		return (print_error("Malloc error in map.grid"));
